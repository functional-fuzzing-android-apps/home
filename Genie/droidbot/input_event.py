...
KEY_ExitEvent = "exit"
#  add a fresh restart event: restart the app with a fresh state
...
KEY_CoordinateTouchEvent = "coordinate_touch"  # Ljx add touch by coordinate
KEY_CoordinateLongTouchEvent = "coordinate_longtouch"  # Ljx add long_touch by coordinate
...
class InputEvent(object):
    ...
    def __init__(self):
        ...
        self.children_event_ids: List[str] = []
        self.parent_event_ids: List[str] = []
        self.weight = 100
        # the event's weight before the update due to this event was executed.
        self.last_weight = 0
        self.number_of_unexecuted_children_events = 0
    ...
    def from_dict(event_dict):
        ...
        elif event_type == KEY_FRESH_RESTART:
            return RestartEvent(event_dict=event_dict)
        elif event_type == KEY_NOP:
            return NOPEvent(event_dict=event_dict)
        elif event_type == KEY_SpawnEvent:
            return SpawnEvent(event_dict=event_dict)
        elif event_type == KEY_CoordinateTouchEvent:
            return CoordinateTouchEvent(event_dict=event_dict)
        elif event_type == KEY_CoordinateLongTouchEvent:
            return CoordinateLongTouchEvent(event_dict=event_dict)

    @abstractmethod
    def get_event_str(self, state):
        pass

    def get_event_signature(self, view_id_free=False):
        return ""

    def get_views(self):
        return []

    def incr_execution_times(self):
        self.executed_times += 1

    def get_execution_times(self):
        return self.executed_times

    def get_current_event_weight(self):
        return self.weight

    def get_last_event_weight(self):
        return self.last_weight

    def set_last_event_weight(self, last_weight):
        self.last_weight = last_weight

    def set_current_event_weight(self, new_weight):
        self.weight = new_weight

    def add_child_event_id(self, child_event_id):
        if child_event_id in self.children_event_ids:
            pass
        else:
            self.children_event_ids.append(child_event_id)

    def add_parent_event_id(self, parent_event_id):
        if parent_event_id in self.parent_event_ids:
            pass
        else:
            self.parent_event_ids.append(parent_event_id)

    def get_children_events(self):
        return self.children_event_ids

    def get_parent_events(self):
        return self.parent_event_ids
...
class EventLog(object):
    ...
    unique_event_log_id = 0

    def __init__(self, device, app, event, event_str=None, profiling_method=None, tag=None,
                 is_inserted_event=False, utg_event_id=0, event_log_json_file_path=None, event_views_file_path=None,
        ...
        if tag is None:
            ...
            #  append with a unique event log id
            tag += '-' + str(EventLog.unique_event_log_id)
            EventLog.unique_event_log_id += 1
        ...
        #
        self.from_state_structure_str = None
        self.to_state_structure_str = None
        self.event_structure_str = None
        self.color_tag = None

        # flag that indicates whether the event is inserted,
        #   False: original event;
        #   True: inserted event
        self.is_inserted_event: bool = is_inserted_event

        # flag that indicates the event id in the utg
        self.utg_event_id = utg_event_id

        # the file path of the event log
        self.event_log_json_file_path = event_log_json_file_path

        # the file path of the event's views
        self.event_views_file_path = [] if event_views_file_path is None else event_views_file_path

        # This field stores the independent views (in form of view ids) of EventLog's from_state.
        self.independent_views: Optional[Set[int]] = None
    ...
    #
    def recover_after_deepcopy(self, device, app):
    ...
    def to_dict(self):
        return {
            ...
            "is_inserted_event": self.is_inserted_event,
            "utg_event_id": self.utg_event_id
        }

    #
    def map_to_cluster_utg(self):
        """
        map the event log onto cluster utg
        :return:
        """
        self.from_state_structure_str = self.from_state.structure_str
        self.to_state_structure_str = self.to_state.structure_str
        self.event_structure_str = self.event.get_event_str(self.from_state, content_free=True)

    #
    def set_color_tag(self, color_tag):
        self.color_tag = color_tag

    def set_insertion_tag(self, is_inserted=False):
        self.is_inserted_event = is_inserted

    def set_utg_event_id(self, event_id):
        self.utg_event_id = event_id
    ...
    def save2dir(self, output_dir):
        ...
        try:
            ...
            self.event_log_json_file_path = event_json_file_path
    ...
    def save_views(self, output_dir):
        # Save views
        views = self.event.get_views()
        if views:
            for view_dict in views:
                view_file_path = self.from_state.save_view_img(view_dict, output_dir)
                self.event_views_file_path.append(view_file_path)
    ...
    def run(self, output_dir, event_interval=1):
        if self is None:
            return
        self.start()
        while True:
            ...
            if not self.device.pause_sending_event:
                break
        self.stop(output_dir)

    def start(self):
        """
        start sending event
        """
        # [Important Fix]: the original code will get the current state again, which is not necessary.
        #   Instead, we directly use the last_know_state, where the event is generated from.
        #   This ensures (1) the constructed utg is always connected, and (2)from the utg file, we can always safely
        #       find the correct mapping between the utg event and the event log files via <from_state, to_state>.
        # self.from_state = self.device.get_current_state()
        self.from_state = self.device.last_know_state

        self.start_profiling()
        self.event_str = self.event.get_event_str(self.from_state)
        print("Input: %s" % self.event_str)
        # print("before event execution: %s" % datetime.datetime.now())
        self.device.send_event(self.event)
        # print("after event execution: %s" % datetime.datetime.now())
    ...
    def stop(self, output_dir):
        """
        :param output_dir: the dir to output EventLog
        finish sending event
        """
        self.stop_profiling()
        self.to_state = self.device.get_current_state(output_dir)
        #  put save_views(...) before save2dir() to make sure all info of event log can be safely
        #       dumped into the event log json file, which later will be used for recovery
        self.save_views(output_dir)
        self.save2dir(output_dir)

    def update_to_state_of_event_log(self, to_state, output_dir):
        """
        update the event log's to_state if necessary
        :param to_state: the new to_state
        :param output_dir:
        :return:
        """
        self.to_state = to_state
        # delete the obsolete event log json file
        if os.path.exists(self.event_log_json_file_path):
            os.remove(self.event_log_json_file_path)
        self.save2dir(output_dir)
...
class ManualEvent(InputEvent):
    ...
    def __init__(self, event_dict=None):
        super(ManualEvent, self).__init__()
...
class NOPEvent(InputEvent):
    """
    an event that do nothing
    """

    def __init__(self, event_dict=None):
        super(NOPEvent, self).__init__()
        self.event_type = KEY_NOP
        if event_dict is not None:
            self.__dict__.update(event_dict)
    ...
    def get_event_str(self, state, content_free=None):
        return "%s()" % self.__class__.__name__
...
class RestartEvent(InputEvent):
    ...
    def __init__(self, intent=None, package=None, permissions=None, event_dict=None):
    ...
    def send(self, device):
        device.clear_data_and_restore_permissions(self.package, self.permissions)
...
class KeyEvent(InputEvent):
    ...
    def get_event_str(self, state, content_free=False):
        return "%s(state=%s, name=%s)" % (self.__class__.__name__,
                                          (state.state_str if not content_free else state.structure_str),
                                          self.name)
...
class TouchEvent(UIEvent):
    ...
    def get_event_str(self, state, content_free=False):
        ...
        :param content_free: The default value is False, which indicates use state_str. Otherwise, use structure_str
        :return:
        """
        if self.view is not None:
            #  change output from 'view_str' to 'signature'
            return "%s(state=%s, view=%s)" % (self.__class__.__name__,
                                              (state.state_str if not content_free else state.structure_str),
                                              self.view['view_str'])
            # return "%s(state=%s, view=%s)" % (self.__class__.__name__, state.state_str, self.view['signature'])
        elif self.x is not None and self.y is not None:
            return "%s(state=%s, x=%s, y=%s)" % (self.__class__.__name__,
                                                 (state.state_str if not content_free else state.structure_str),
                                                 self.x, self.y)
        else:
    ...
    def get_event_signature(self, view_id_free=False):
        """
        get the event signature (do not consider state info)
        :return: str
        """
        if self.view is not None:
            if view_id_free:
                view_str = self.view['view_str']
                underscore_position = view_str.find('_')
                if underscore_position != -1:
                    view_str = view_str[0:underscore_position]
            else:
                view_str = self.view['view_str']
            return "%s(view=%s)" % (self.__class__.__name__, view_str)
        elif self.x is not None and self.y is not None:
...
class LongTouchEvent(UIEvent):
    ...
    def __init__(self, x=None, y=None, view=None, duration=1500, event_dict=None):
        super(LongTouchEvent, self).__init__()
        ...
        if self.view is not None:
            return "%s(state=%s, view=%s, duration=%s)" % \
                   (self.__class__.__name__,
                    (state.state_str if not content_free else state.structure_str),
                    self.view['view_str'], self.duration)
            # return "%s(state=%s, view=%s, duration=%s)" % \
            #       (self.__class__.__name__, state.state_str, self.view['signature'], self.duration)
        elif self.x is not None and self.y is not None:
    ...
    def get_event_signature(self, view_id_free=False):
        ...
        if self.view is not None:
            if view_id_free:
                ...
                view_str = self.view['view_str']
            return "%s(view=%s, duration=%s)" % \
                   (self.__class__.__name__, view_str, self.duration)
        elif self.x is not None and self.y is not None:
            return "%s(x=%s, y=%s, duration=%s)" % \
                   (self.__class__.__name__,
                    self.x, self.y, self.duration)
        else:
            msg = "Invalid %s!" % self.__class__.__name__
...
class CoordinateTouchEvent(UIEvent):
    """
       a coordinate touch on screen
    """

    def __init__(self, x=None, y=None, view=None, event_dict=None):
        super(CoordinateTouchEvent, self).__init__()
        self.event_type = KEY_CoordinateTouchEvent
        self.x = x
        self.y = y
        self.view = view
        if event_dict is not None:
...
class CoordinateLongTouchEvent(UIEvent):
    """
       a coordinate long touch on screen
    """

    def __init__(self, x=None, y=None, view=None, duration=2000, event_dict=None):
        super(CoordinateLongTouchEvent, self).__init__()
        self.event_type = KEY_CoordinateLongTouchEvent
        self.x = x
        self.y = y
        self.view = view
        self.duration = duration
...
class SwipeEvent(UIEvent):
    ...
    def __init__(self,
        ...
        super(SwipeEvent, self).__init__()
    ...
    def get_event_str(self, state, content_free=False):
        if self.start_view is not None:
            start_view_str = "state=%s, start_view=%s" % \
                             ((state.state_str if not content_free else state.structure_str),
                              self.start_view['view_str'])
    ...
    def get_event_signature(self, view_id_free=False):
        if self.start_view is not None:
            ...
            if view_id_free:
                view_str = self.start_view['view_str']
                underscore_position = view_str.find('_')
                if underscore_position != -1:
                    view_str = view_str[0:underscore_position]
            else:
            ...
            start_view_str = "start_view=%s" % view_str
        elif self.start_x is not None and self.start_y is not None:
            start_view_str = "start_x=%s, start_y=%s" % \
                             (self.start_x,
                              self.start_y)
        else:
            msg = "Invalid %s!" % self.__class__.__name__
        ...
        if self.end_view is not None:
            if view_id_free:
                view_str = self.end_view['view_str']
                underscore_position = view_str.find('_')
                if underscore_position != -1:
                    view_str = view_str[0:underscore_position]
            else:
                view_str = self.end_view['view_str']
            end_view_str = "end_view=%s" % view_str
        elif self.end_x is not None and self.end_y is not None:
            end_view_str = "end_x=%s, end_y=%s" % (self.end_x, self.end_y)
        else:
            msg = "Invalid %s!" % self.__class__.__name__
    ...
    #
    def set_views(self, views):
        self.start_view = views[0]
        self.end_view = views[1]
    ...
    def get_event_signature(self, view_id_free=False):
        if self.view is not None:
            if view_id_free:
                ...
                view_str = self.view['view_str']
            return "%s(view=%s, direction=%s)" % \
                   (self.__class__.__name__,
                    view_str,
                    self.direction)
        elif self.x is not None and self.y is not None:
            return "%s(x=%s, y=%s, direction=%s)" % \
                   (self.__class__.__name__,
                    self.x, self.y, self.direction)
        else:
            return "%s(direction=%s)" % \
                   (self.__class__.__name__,
                    self.direction)
...
class SetTextEvent(UIEvent):
    ...
    def __init__(self, x=None, y=None, view=None, text=None, event_dict=None):
        super(SetTextEvent, self).__init__()
    ...
    def get_event_str(self, state, content_free=False):
        if self.view is not None:
            return "%s(state=%s, view=%s, text=%s)" % \
                   (self.__class__.__name__,
                    (state.state_str if not content_free else state.structure_str),
                    self.view['view_str'], self.text)
            # return "%s(state=%s, view=%s, text=%s)" % \
            #   (self.__class__.__name__, state.state_str, self.view['signature'], self.text)
        elif self.x is not None and self.y is not None:
    ...
    def get_event_signature(self, view_id_free=False):
        if self.view is not None:
            if view_id_free:
                ...
                view_str = self.view['view_str']
            return "%s(view=%s, text=%s)" % \
                   (self.__class__.__name__,
                    view_str, self.text)
        elif self.x is not None and self.y is not None:
            return "%s(x=%s, y=%s, text=%s)" % \
                   (self.__class__.__name__,
                    self.x, self.y, self.text)
        else:
            msg = "Invalid %s!" % self.__class__.__name__
...
class IntentEvent(InputEvent):
    ...
    def __init__(self, intent=None, event_dict=None):
        super(IntentEvent, self).__init__()
        self.event_type = KEY_IntentEvent
        if isinstance(intent, Intent):
            self.intent = intent.get_cmd()
        elif isinstance(intent, str):
            self.intent = intent
        #  I lifted the check of event_dict before throwing the exception.
        #   This seems fixed a bug.
        elif event_dict is not None:
            self.__dict__.update(event_dict)
        else:
            msg = "intent must be either an instance of Intent or a string."
...
class SpawnEvent(InputEvent):
    ...
    def __init__(self, event_dict=None):
        super(SpawnEvent, self).__init__()
