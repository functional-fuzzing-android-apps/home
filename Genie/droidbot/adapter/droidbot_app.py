...
class DroidBotAppConn(Adapter):
    ...
    def set_up(self):
        ...
        while ACCESSIBILITY_SERVICE not in device.get_service_names() and self.__can_wait:
            print("Please wait or enable accessibility for DroidBot app manually.")
    ...
    def connect(self):
        ...
        try:
            ...
            subprocess.check_call(forward_cmd.split(), stdout=subprocess.DEVNULL)
    ...
    # In this function, it converts the view tree into a view list in a depth-first order.
    # In this view list, each element is a view with its 'temp_id', 'children', 'child_count', 'size' and 'bounds'
    # Compared to the info dumped by UIAutomatorView, the accessibility service provides some additional info about
    #  a view, i.e., 'visible' and 'editable'
        ...
        for child_tree in view_tree['children']:
            if self.ignore_ad and child_tree['resource_id'] is not None:
                ...
                if "ad" in id_word_list or \
                    ...
                    continue
            # Remove invisible views at the very beginning (when receiving the view tree from the Droidbot app)
            #   Note Droidbot app by default may dump invisible views.
            #   This issue may lead to (1) some false positives when oracle checking
            #                          (2) two different pages are computed as same
            if child_tree['visible'] is False:
                continue
    ...
    def get_views(self):
        ...
        # if this view was previously accessed, then directly return the previously computed "view_list"
        ...
        if not view_tree:
            self.logger.warning("the view tree is null!")
            return None
        view_tree['parent'] = -1
        view_list = []
