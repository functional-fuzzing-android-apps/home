...
#
def init_visualization_env(output_dir):
    html_index_path = pkg_resources.resource_filename("droidbot", "resources/index.html")
    shutil.copy(html_index_path, output_dir)


def replace_file(f: str, file_handler: Callable[[str], str]):
    with open(f, 'r+') as f:
        assert f.readable() and f.writable() and f.seekable()
        new_text = file_handler(f.read())
        f.seek(0)
        f.write(new_text)
        f.truncate()
...
def delete_files(files_to_delete):
    for file in files_to_delete:
        try:
            ...
            print("Error when deleting file: %s" % file)
